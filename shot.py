...
class Shot:
    """
    تمثيل التسديقة الكاملة
    
    Attributes:
        rails (int): عدد الجدران (1-4)
        cue_position (float): موضع العصا على الدايمند
        white_ball (float): موضع الكرة البيضاء
        target (float): موضع الكرة المستهدفة
        pocket (int): موضع الجيب المستهدف (0-5)
        difficulty (Difficulty): مستوى الصعوبة
        success_rate (float): معدل النجاح (0-100)
        executed (bool): هل تم تنفيذ التسديقة؟
    """
    ...
    def validate(self) -> bool:
        ...
        if not (0 <= self.success_rate <= 100):
            raise ValueError(f"معدل النجاح يجب أن يكون بين 0 و 100")
        
        return True
    
    def to_dict(self) -> dict:
        """تحويل التسديقة إلى قاموس"""
        return {
            'rails': self.rails,
